...
describe "Follow-up parent journey" do
  ...
  scenario "Nurse follows up and confirms the refusal" do
    ...
    and_i_see_email_and_sms_notifications_in_the_activity_log
  end
  ...
  def and_i_see_email_and_sms_notifications_in_the_activity_log
    new_consent = @patient.notify_log_entries.includes(:consent).last.consent
    outcome = new_consent&.response_given? ? "give consent" : "confirm refusal"
    within(".app-card", text: "Programme activity") do
      expect(page).to have_content(
        "Confirmation of follow-up decision to #{outcome} sent to " \
          "#{new_consent&.name} (#{new_consent&.who_responded&.downcase_first})"
      )
    end
  end
end
